...
export function KeysClient() {
  ...
  useEffect(() => { let active = true; void Promise.all([manageFetch<VirtualKey[]>("keys"), manageFetch<RouteRecord[]>("routes")]).then(([keyData, routeData]) => { if (active) applyData(keyData, routeData); }).catch((err) => { if (active) setError(err instanceof Error ? err.message : "API keys could not be loaded."); }); return () => { active = false; }; }, [applyData]);
  ...
  return (
    <div className="sr-stack key-page">
      <div className="sr-page-header"><div><p className="sr-kicker">04 / ROUTE-BOUND ACCESS</p><h1>API Keys</h1><p>Every SwitchRoute key belongs to one Route and one environment. Full secrets are displayed exactly once.</p></div><Button disabled={!routes.length} onClick={() => setAdding(true)}>Create API key</Button></div>
