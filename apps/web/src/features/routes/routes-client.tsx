...
export function RoutesClient() {
  ...
  useEffect(() => { let active = true; void Promise.all([manageFetch<RouteRecord[]>("routes"), manageFetch<ProviderConnection[]>("providers")]).then(([routeData, providerData]) => { if (active) applyData(routeData, providerData); }).catch((err) => { if (active) setError(err instanceof Error ? err.message : "Routes could not be loaded."); }); return () => { active = false; }; }, [applyData]);
  ...
  return (
    <div className="sr-stack route-page">
      <div className="sr-page-header"><div><p className="sr-kicker">03 / REQUEST SELECTION</p><h1>Routes</h1><p>An ordered provider/model priority stack. One request goes to one target; fallback moves downward only when needed.</p></div><Button disabled={!providers.length} onClick={() => setEditing("new")}>Create Route</Button></div>
