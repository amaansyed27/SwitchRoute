...
export function RoutesClient() {
  ...
  const load = useCallback(async () => { setError(null); try { const [routeData, providerData] = await Promise.all([manageFetch<RouteRecord[]>("routes"), manageFetch<ProviderConnection[]>("providers")]); setRoutes(routeData); setProviders(providerData); } catch (err) { setError(err instanceof Error ? err.message : "Waterfalls could not be loaded."); } finally { setLoading(false); } }, []);
