...
export function RouteForm({ providers, route, onSaved }: { providers: ProviderConnection[]; route?: RouteRecord; onSaved: () => void }) {
  ...
  const [paidFallback, setPaidFallback] = useState<PaidFallback>(route?.paid_fallback ?? "never");
  ...
  return <form onSubmit={submit} className="space-y-6">
    ...
    <details><summary className="mb-3 cursor-pointer text-sm font-medium">Routing strategy: {strategies.find((item) => item.id === strategy)?.title}</summary><div className="grid gap-2 sm:grid-cols-2">{strategies.map((item) => <button key={item.id} type="button" aria-pressed={strategy === item.id} onClick={() => setStrategy(item.id)} className={cn("rounded-xl border p-3 text-left transition", strategy === item.id ? "border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_7%,var(--surface))]" : "border-[var(--border)] bg-[var(--surface)] hover:border-[var(--border-strong)]")}><strong className="text-sm">{item.title}</strong><p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">{item.body}</p></button>)}</div></details>
    <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)]"><h3 className="px-3 py-3 text-sm font-medium">Spending permissions</h3><div className="grid gap-4 border-t border-[var(--border)] p-3 sm:grid-cols-2"><Field label="Paid fallback" htmlFor="paid-fallback"><Select id="paid-fallback" value={paidFallback} onChange={(event) => setPaidFallback(event.target.value as PaidFallback)}><option value="never">Never use paid</option><option value="after_free">After free capacity</option><option value="allowed">Paid allowed</option></Select></Field><Field label="Daily paid cap (USD)" htmlFor="daily-cap"><Input id="daily-cap" inputMode="decimal" min="0" step="0.01" value={dailyCap} onChange={(event) => setDailyCap(event.target.value)} placeholder="No cap"/></Field><p className="text-xs leading-5 text-[var(--muted-foreground)] sm:col-span-2">Account-dependent means the provider offers different billing plans; it does not confirm free access for this model or key. With Never use paid, only confirmed free capacity can run. Paid and unverified targets are blocked.</p></div></section>
    ...
    <div><div className="mb-3 flex items-end justify-between gap-3"><div><p className="text-xs font-medium">Targets</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">Requests try the first eligible model, then its backups. Use the arrows to reorder.</p></div><Button variant="secondary" size="sm" type="button" disabled={!providers.some((item) => item.metadata.models?.length)} onClick={addTarget}><Icon name="plus" className="size-3.5"/>Add model</Button></div><TargetStack targets={targets} providers={providers} onChange={setTargets}/>{!targets.length && <div className="mt-3"><Alert tone="info">Add at least one provider/model target.</Alert></div>}</div>
    {paidFallback === "never" && targets.some((target) => target.billing_tier !== "free") && <Alert tone="info">Some selected targets need confirmed free account capacity. The gateway will skip them if free capacity cannot be verified. Choose a confirmed free model or explicitly allow paid requests.</Alert>}
    {error && <Alert>{error}</Alert>}
    <div className="flex justify-end border-t border-[var(--border)] pt-4"><Button type="submit" disabled={busy || !name.trim() || !slug || !targets.length || targets.some((target) => !target.model_id)}>{busy ? "Saving…" : route ? "Save changes" : "Create waterfall"}</Button></div>
  </form>;
}
