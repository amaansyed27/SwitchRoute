...
export function Drawer({ title, description, onClose, children, wide = false }: { title: string; description?: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  return <div className="sr-drawer-backdrop fixed inset-0 z-50 flex justify-end bg-black/35 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label={title} className={`sr-drawer-panel ${wide ? "max-w-3xl" : "max-w-xl"} h-full w-full overflow-y-auto border-l border-[var(--border)] bg-[var(--background)] shadow-2xl`}>
