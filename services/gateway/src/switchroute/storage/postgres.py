...
class PostgresRepository:
    ...
    async def _write_route(
        ...
        if route_id:
            row = await conn.fetchrow(
                "update public.routes set name=$3,slug=$4,strategy=$5,enabled=$6,updated_at=now() where id=$1 and workspace_id=$2 returning *",
                route_id,
                workspace_id,
                name,
                slug,
        ...
        for index, target in enumerate(targets):
            result = await conn.execute(
                "insert into public.route_targets(route_id,provider_connection_id,model_id,position,billing_tier,enabled) select $1,$2,$3,$4,$5,$6 where exists(select 1 from public.provider_connections where id=$2 and workspace_id=$7)",
                route_id,
    ...
    async def create_route(
        ...
        async with self._require_pool().acquire() as conn, conn.transaction():
            return await self._write_route(
                conn, workspace_id, None, name, slug, strategy, enabled, targets
            )
    ...
    async def update_route(
        ...
        async with self._require_pool().acquire() as conn, conn.transaction():
            return await self._write_route(
                conn, workspace_id, route_id, name, slug, strategy, enabled, targets
            )
    ...
    async def delete_route(self, workspace_id: UUID, route_id: UUID) -> None:
        try:
            ...
            raise SwitchRouteError(
                "invalid_request", "Revoke Route API keys before deleting this Route.", 409
            ) from exc
    ...
    async def create_key(
        ...
        row = await self._require_pool().fetchrow(
            "insert into public.virtual_api_keys(workspace_id,route_id,environment,name,prefix,key_hash,expires_at) select $1,id,$3,$4,$5,$6,$7::timestamptz from public.routes where id=$2 and workspace_id=$1 returning id,name,prefix,environment,status,route_id,created_at,expires_at",
            workspace_id,
            route_id,
            environment,
            name,
    ...
    async def revoke_key(self, workspace_id: UUID, key_id: UUID) -> None:
        await self._require_pool().execute(
            "update public.virtual_api_keys set status='revoked',revoked_at=now() where id=$1 and workspace_id=$2",
            key_id,
            workspace_id,
        )
    ...
    async def resolve_virtual_key(self, key_hash: str) -> VirtualKeyContext | None:
        ...
        key = await pool.fetchrow(
            """select k.id key_id,k.workspace_id,k.route_id,r.name route_name,r.slug route_slug,r.strategy,r.enabled route_enabled
        ...
        rows = await pool.fetch(
            """select t.id target_id,t.provider_connection_id,p.provider_kind,t.model_id,t.billing_tier,t.position
    ...
    async def mark_key_used(self, key_id: UUID) -> None:
        await self._require_pool().execute(
            "update public.virtual_api_keys set last_used_at=now() where id=$1", key_id
        )

    async def record_usage(self, record: UsageRecord) -> None:
        await self._require_pool().execute(
            """insert into public.request_usage(request_id,workspace_id,route_id,virtual_key_id,provider_connection_id,provider_kind,model_id,input_tokens,output_tokens,latency_ms,status,fallback_count,estimated_cost_microusd,error_category)
    ...
    async def activity(
        ...
        rows = await self._require_pool().fetch(
            """select u.request_id,u.created_at,r.name route_name,u.provider_kind,u.model_id,u.input_tokens,u.output_tokens,u.latency_ms,u.status,u.fallback_count,u.error_category
          from public.request_usage u join public.routes r on r.id=u.route_id where u.workspace_id=$1 order by u.created_at desc limit $2""",
            workspace_id,
            min(max(limit, 1), 100),
        )
        return [dict(row) for row in rows]
